...
export function Settings() {
  ...
  const [printerStatus, setPrinterStatus] = useState<PrinterStatusData | null>(
    null,
  );
  ...
  const sendTestPrint = async (printer: string) => {
    ...
    try {
      ...
      if (data.success) {
        setTestMessage({
          type: "success",
          text: `Test print sent to ${printer} printer!`,
        });
        ...
        setTestMessage({
          type: "error",
          text: data.error || `Failed to send test print to ${printer}`,
        });
      }
    } catch (error) {
      setTestMessage({
      ...
      <div className="flex gap-8">
        ...
        <div className="flex-1 bg-white rounded-3xl border border-zinc-200 shadow-sm p-8 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-blue-600 bg-blue-50 p-3 rounded-xl">
                ...
                <span className="text-sm font-medium">
                  Restaurant information is configured by the system
                  administrator.
                </span>
              </div>
          ...
          {activeSection === "tax" && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-blue-600 bg-blue-50 p-3 rounded-xl">
                <Info size={18} />
                <span className="text-sm font-medium">
                  Tax rates are fixed and cannot be changed from this interface.
                </span>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <p className="text-xs text-amber-700 font-medium">
                  ℹ️ Tax rate is applied to all orders automatically. To change
                  tax rates, please contact your system administrator.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
                  ...
                  {printerStatus?.bridgeConnected ? (
                    <>
                      ...
                      <span className="text-sm text-zinc-600">
                        Connected to {printerStatus.bridgeUrl}
                      </span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-sm text-red-600">
                        Bridge not connected - check if bridge service is
                        running
                      </span>
                    </>
                  )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">
                        IP:{" "}
                        {printerStatus?.printers?.kitchen?.ip ||
                          "192.168.1.127"}
                      </p>
                    </div>
                  </div>
                  {printerStatus?.printers?.kitchen &&
                    (() => {
                      const badge = getPrinterStatusBadge(
                        printerStatus.printers.kitchen.status,
                      );
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">
                        IP:{" "}
                        {printerStatus?.printers?.bar?.ip || "192.168.1.136"}
                      </p>
                    </div>
                  </div>
                  {printerStatus?.printers?.bar &&
                    (() => {
                      const badge = getPrinterStatusBadge(
                        printerStatus.printers.bar.status,
                      );
                      const BadgeIcon = badge.icon;
                      return (
                        <div
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-400">
                        USB:{" "}
                        {printerStatus?.printers?.customer?.name ||
                          "Epson TM-U220B"}
                      </p>
                    </div>
                  </div>
                  {printerStatus?.printers?.customer &&
                    (() => {
                      const badge = getPrinterStatusBadge(
                        printerStatus.printers.customer.status,
                      );
                      const BadgeIcon = badge.icon;
                      return (
                        <div
              ...
              {printerStatus?.lastPoll && (
                <p className="text-xs text-zinc-400 text-center">
                  Last updated:{" "}
                  {new Date(printerStatus.lastPoll).toLocaleTimeString()}
                </p>
              )}
            </div>
          )}
          ...
          {activeSection === "payment" && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-zinc-900 text-center py-12 text-zinc-300 italic">
                Payment gateway integration coming in v2.0
              </h3>
            </div>
          )}
        </div>
